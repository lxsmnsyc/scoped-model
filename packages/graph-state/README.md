...
* `get`: acts as a default value. If a function is supplied, the function is lazily evaluated as it enters the `GraphDomain` to provide the default value. This function may also be invoked again if the dependencies update. The function an object with 3 fields:
  * `{ get, set, subscription }`: An interface for controlling graph nodes.
